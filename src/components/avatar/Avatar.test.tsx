...
describe("Avatar", () => {
  ...
  it("applies default classes (md circle)", () => {
  ...
  it("applies variant classes (success uses badge token vars)", () => {
    render(<Avatar initials="OK" variant="success" />);
    const avatar = screen.getByText("OK").closest("[data-slot='avatar']");
    // Token-driven class string check (bg var)
    expect(avatar?.className).toContain("--atom-badge-success-bg-low");
  });

  it("applies appearance compound (solid + success uses high tokens)", () => {
    render(<Avatar initials="OK" variant="success" appearance="solid" />);
    const avatar = screen.getByText("OK").closest("[data-slot='avatar']");
    expect(avatar?.className).toContain("--atom-badge-success-bg-high");
  });
  ...
  it("sets data-slot=avatar attribute", () => {
    render(<Avatar initials="AB" />);
    expect(
      screen.getByText("AB").closest("[data-slot='avatar']")
    ).toBeInTheDocument();
  });
