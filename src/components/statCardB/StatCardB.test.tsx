...
import { StatCardB } from "./StatCardB";
import { cn } from "../../lib/cn";

describe("StatCardB", () => {
type StatCardBAllProps =
  Omit<React.ComponentPropsWithRef<typeof StatCardB>, "label" | "value"> & {
    label: string;
    value: React.ReactNode;
  };


const renderStatCardB = (props: StatCardBAllProps) => {
  return render(
    <StatCardB data-testid="stat-card" {...props} />
  );
};
  ...
  it("renders basic StatCard with required props", () => {
    renderStatCardB({ label: "Validated", value: "42" });
  ...
  it("applies default variants", () => {
    renderStatCardB({ label: "Total", value: "100" });
  ...
  it("applies size md (default)", () => {
    renderStatCardB({ label: "Medium", value: "25" });
    const card = getStatCard();
  ...
  it("applies size lg", () => {
    renderStatCardB({ size: "lg", label: "Large", value: "50" });
    const card = getStatCard();
  ...
  it("applies appearance elevated (default)", () => {
    renderStatCardB({ label: "Elevated", value: "42" });
    const card = getStatCard();
    expect(card).toHaveClass("shadow-sm");
  });

  it("applies appearance outlined", () => {
    renderStatCardB({ appearance: "outlined", label: "Outlined", value: "15" });
    const card = getStatCard();
  ...
  it("applies appearance ghost", () => {
    renderStatCardB({ appearance: "ghost", label: "Ghost", value: "7" });
    const card = getStatCard();
    expect(card).toHaveClass("shadow-none");
    expect(card).toHaveClass("bg-transparent");
  ...
  it("applies appearance soft", () => {
    renderStatCardB({ appearance: "soft", label: "Soft", value: "33" });
    const card = getStatCard();
    expect(card).toHaveClass("shadow-none");
  ...
  it("applies fullWidth true", () => {
    renderStatCardB({ fullWidth: true, label: "Full", value: "100%" });
    const card = getStatCard();
  ...
  it("renders icon correctly", () => {
    ...
    renderStatCardB({ label: "With Icon", value: "42", icon: TestIcon });
  ...
  it("renders label with correct container structure", () => {
    renderStatCardB({ label: "Test Label", value: "123" });
  ...
  it("renders value with correct styling", () => {
    renderStatCardB({ label: "Value", value: <span data-testid="value">999</span> });
  ...
  it("renders with asChild=false (default div)", () => {
    renderStatCardB({ label: "Default", value: "42" });
    const card = getStatCard();
  ...
  it("renders with asChild=true using Slot", () => {
    ...
    render(
      <TestLink>
        <StatCardB asChild label="Clickable" value="42" />
      </TestLink>
    );
  ...
  it("applies data-slot attribute", () => {
    renderStatCardB({ label: "Slot Test", value: "99" });
    const card = getStatCard();
